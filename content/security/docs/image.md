...
### Use images that publish unpatched CVEs
When using an image vulnerability scanner, it’s important to use a base image that publishes both patched and unpatched CVEs. Scanners work off of the CVE lists managed by the image owners, and only some platforms publish known vulnerabilities that remain unpatched. If your base image does not publish unpatched vulnerabilities, you can end up in a situation where attackers are aware of the issue, but your image scanner isn't. See this chart from Aqua Security's Trivy for details on which images publish unpatched CVEs: https://github.com/aquasecurity/trivy#os-packages
